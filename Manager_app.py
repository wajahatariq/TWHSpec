...
if "Date of Charge" not in df.columns:
    st.error("No 'Date of Charge' column found in Spectrum sheet.")
else:
    df["Date of Charge"] = pd.to_datetime(df["Date of Charge"], errors="coerce", dayfirst=True)

    # --- Determine dynamic 15th → today range ---
    ...
    # --- Filter the data ---
    filtered_df = df[(df["Date of Charge"].notna()) &
                     (df["Date of Charge"] >= start_date) &
                     (df["Date of Charge"] <= end_date)]

    # --- Fallback if no rows in range ---
    if filtered_df.empty:
        filtered_df = df[df["Date of Charge"].notna()]
        st.info(f"No transactions in {start_date.date()} → {end_date.date()}, using all available data ({len(filtered_df)} rows).")
    ...
    if not filtered_df.empty:
        ...
        # --- Base AI prompt ---
        base_ai_prompt = f"""
You are a senior financial analyst. You have access to Spectrum transaction data from {filtered_df['Date of Charge'].min().date()} to {filtered_df['Date of Charge'].max().date()}.
