...
with main_tab3:
    ...
    if not df.empty:
        # Ensure numeric column exists before filtering
        ...
        df = df[df["Transaction Date"] >= cutoff_date]
    
        if df.empty:
            st.warning("No data available for the last 15 days.")
        else:
            # --- Filter Controls ---
